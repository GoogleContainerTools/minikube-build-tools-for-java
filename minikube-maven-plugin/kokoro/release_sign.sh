...
# find the latest directory under prod/minikube-build-tools-for-java/maven/gcp_ubuntu/release-build/
LAST_BUILD=$(ls prod/minikube-build-tools-for-java/maven/gcp_ubuntu/release-build/ | sort -rV | head -1)

# find the jars and the pom in the latest build artifact directory
FILES=$(find `pwd`/prod/minikube-build-tools-for-java/maven/gcp_ubuntu/release-build/${LAST_BUILD}/* -type f \( -iname \*.jar -o -iname \*.pom \))
